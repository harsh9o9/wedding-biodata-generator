...
Templates define the visual appearance of the biodata. Each template is a React component that renders:

1. **Preview Component**: HTML/CSS component for both live preview and PDF generation
2. **Configuration**: Color scheme, fonts, and layout settings

PDFs are generated by capturing the preview component using html2canvas and converting to PDF with jsPDF.
...
### Step 2: Add Template Thumbnail

Create a thumbnail image for template selection:
- Size: 300x400px
- Place in: `public/templates/custom-thumb.png`
- Show representative preview of the template
...
- Adjust scale parameter in html2canvas options (default: 2)
- Increase quality parameter (0-1, default: 0.92)
- Ensure preview element has proper dimensions

**2. Fonts not rendering in PDF**
- Verify Google Fonts are loaded in preview
- Check browser console for font loading errors
- Fonts are automatically captured by html2canvas

**3. Color mismatch between preview and PDF**
- PDF uses JPEG compression which may slightly alter colors
- Test with different quality settings
- Use web-safe colors for consistency

**4. Layout shifts on different data**
- Test with empty fields
- Test with maximum length content
- Use fixed widths where appropriate

**5. Large file sizes**
- Default quality (0.92) produces 200-500KB files
- Reduce quality parameter if smaller files needed
- Optimize background images before upload
